...
impl<R: rand::RngCore> Scru128Generator<R> {
    ...
    pub fn generate_core(&mut self, timestamp: u64) -> Scru128Id {
        if let Some(value) = self.generate_core_monotonic(timestamp) {
            value
        } else {
            // reset state and resume
            self.timestamp = 0;
            self.ts_counter_hi = 0;
            let value = self.generate_core_monotonic(timestamp).unwrap();
            self.last_status = Status::ClockRollback;
            value
        }
    }

    /// Generates a new SCRU128 ID object from the `timestamp` passed, guaranteeing that the
    /// returned ID is greater than the immediately preceding one generated by the same generator.
    ///
    /// This method returns monotonically increasing IDs unless a given `timestamp` is
    /// significantly (by ten seconds or more) smaller than the one embedded in the immediately
    ...
    ///
    /// Panics if the argument is not a 48-bit positive integer.
    pub fn generate_core_monotonic(&mut self, timestamp: u64) -> Option<Scru128Id> {
    ...
    #[deprecated(
        since = "2.6.0",
        note = "use `generate_monotonic()` to guarantee monotonicity"
    )]
...
mod std_ext {
    ...
    impl<R: rand::RngCore> Scru128Generator<R> {
        ...
        /// returned ID is greater than the immediately preceding one generated by the same
        /// generator.
        ///
        /// This method returns monotonically increasing IDs unless the up-to-date `timestamp` is
        /// significantly (by ten seconds or more) smaller than the one embedded in the immediately
...
mod tests_generate_core_monotonic {
    ...
    fn generates_increasing_ids_even_with_decreasing_or_constant_timestamp() {
        ...
        for i in 0..100_000u64 {
            let curr = g.generate_core_monotonic(ts - i.min(9_998)).unwrap();
            assert!(
                g.last_status == Status::CounterLoInc
                    || g.last_status == Status::CounterHiInc
                    || g.last_status == Status::TimestampInc
    ...
    fn returns_none_if_timestamp_moves_backward_a_lot() {
        ...
        let mut curr = g.generate_core_monotonic(ts - 10_000);
        assert!(curr.is_none());
        assert_eq!(g.last_status, Status::NewTimestamp);

        curr = g.generate_core_monotonic(ts - 10_001);
        assert!(curr.is_none());
        assert_eq!(g.last_status, Status::NewTimestamp);
    }
}
